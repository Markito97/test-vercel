import { Button } from "@/components/ui/button";
import { Comic_Neue, Roboto } from "next/font/google";
import React from "react";
import { HeroMarquee } from "@/components/ui/hero-marquee";
import { PossibilitiesList } from "@/components/ui/possibilities-list";
import { WhatYouGetList } from "@/components/ui/what-you-get-list";


const comicNeue = Comic_Neue({
  weight: ['400', '700'],
  subsets: ['latin',],
});

const roboto = Roboto({
  weight: ['400', '500', '700'],
  subsets: ['cyrillic']
})


export default function Home() {
  return (
    <div>
      <main className="relative min-h-screen">
        <section
          className="relative -mt-[65px] pb-14 md:pb-24"
        >
          <div className="sm:py-18 container relative mx-auto px-6 py-16 md:py-24 lg:px-16 lg:py-24 xl:px-20 pt-8 pb-10 md:pt-16 overflow-hidden">
            <div className="relative">
              <div className="mx-auto">
                <div className="mx-auto max-w-2xl lg:col-span-6 lg:flex lg:items-center justify-center text-center">
                  <div className="relative z-10 lg:h-auto pt-[90px] lg:pt-[90px] lg:min-h-[300px] flex flex-col items-center justify-center sm:mx-auto md:w-3/4 lg:mx-0 lg:w-full gap-4 lg:gap-8">
                    <div className="flex flex-col items-center">
                      <h1
                        className={`${comicNeue.className} text-foreground text-4xl sm:text-5xl sm:leading-none lg:text-7xl`}
                      >
                        <span className="block text-foreground">
                          Энергобит
                        </span>
                        <span className="text-brand block md:ml-0">
                          Умное управление энергозатратами
                        </span>
                      </h1>
                      <p className={`${roboto.className} pt-2 text-foreground my-3 text-sm sm:mt-5 lg:mb-0 sm:text-base lg:text-lg`}>
                        Ваш надежный инструмент для оптимизации расходов на электроэнергию.
                        <br /> Благодаря передовым алгоритмам на базе искусственного интеллекта
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button className="bg-brand">
                        Начать
                      </Button>
                      <Button variant='outline'>
                        Демо доступ
                      </Button>
                    </div>
                  </div>

                </div>

              </div>

            </div>

          </div>

        </section>
        <HeroMarquee/>
        <WhatYouGetList/>
        <PossibilitiesList/>
      </main>
    </div>
  );
}




const CalcIcon = () => {
  return <svg
    className="lucide lucide-calculator text-white opacity-30 w-full h-full stroke-current group-hover:stroke-[1.5px] transition-all duration-300 hover:stroke-[10px]"
    xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
    <rect width="16" height="20" x="4" y="2" rx="2" />
    <line x1="8" x2="16" y1="6" y2="6" />
    <line x1="16" x2="16" y1="14" y2="18" />
    <path d="M16 10h.01" />
    <path d="M12 10h.01" />
    <path d="M8 10h.01" />
    <path d="M12 14h.01" />
    <path d="M8 14h.01" />
    <path d="M12 18h.01" />
    <path d="M8 18h.01" />
  </svg>
}

const ForecastIcon = () => {
  return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"
    className="lucide lucide-trending-up-down text-white opacity-30 w-full h-full stroke-current group-hover:stroke-[1.5px] transition-all duration-300 hover:stroke-[10px]"
  >
    <path d="M14.828 14.828 21 21" /><path d="M21 16v5h-5" /><path d="m21 3-9 9-4-4-6 6" /><path d="M21 8V3h-5" /></svg>
}

const ForecastPeakHourIcon = () => {
  return <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="1"
    strokeLinecap="round"
    strokeLinejoin="round"
    className="lucide lucide-chart-no-axes-combined text-white opacity-30 w-full h-full stroke-current group-hover:stroke-[1.5px] transition-all duration-300 hover:stroke-[10px]"
  >
    <path d="M12 16v5" className="svg-path" />
    <path d="M16 14v7" className="svg-path" />
    <path d="M20 10v11" className="svg-path" />
    <path d="m22 3-8.646 8.646a.5.5 0 0 1-.708 0L9.354 8.354a.5.5 0 0 0-.707 0L2 15" className="svg-path" />
    <path d="M4 18v3" className="svg-path" />
    <path d="M8 14v7" className="svg-path" />
  </svg>
}

// const ForecastPeakHourIcon = () => {
//   return (
//     <svg
//       xmlns="http://www.w3.org/2000/svg"
//       width="24"
//       height="24"
//       viewBox="0 0 24 24"
//       fill="none"
//       stroke="currentColor"
//       strokeWidth="2"
//       strokeLinecap="round"
//       strokeLinejoin="round"
//       className="lucide lucide-chart-no-axes-combined text-white opacity-30 w-full h-full stroke-current group-hover:stroke-[1.5px] transition-all duration-300 hover:stroke-[10px]"
//     >
//       <path d="M12 16v5">
//         <animate
//           attributeName="stroke-dasharray"
//           from="0,5"
//           to="5,0"
//           dur="1s"
//           repeatCount="1"
//           fill="freeze"
//         />
//       </path>
//       <path d="M16 14v7">
//         <animate
//           attributeName="stroke-dasharray"
//           from="0,7"
//           to="7,0"
//           dur="1s"
//           begin="0.2s"
//           repeatCount="1"
//           fill="freeze"
//         />
//       </path>
//       <path d="M20 10v11">
//         <animate
//           attributeName="stroke-dasharray"
//           from="0,11"
//           to="11,0"
//           dur="1s"
//           begin="0.4s"
//           repeatCount="1"
//           fill="freeze"
//         />
//       </path>
//       <path d="M22 3L13.354 11.646a.5.5 0 0 1-.708 0L9.354 8.354a.5.5 0 0 0-.707 0L2 15">
//         <animate
//           attributeName="stroke-dasharray"
//           from="0,30"
//           to="30,0"
//           dur="1.5s"
//           begin="0.6s"
//           repeatCount="1"
//           fill="freeze"
//         />
//       </path>
//       <path d="M4 18v3">
//         <animate
//           attributeName="stroke-dasharray"
//           from="0,3"
//           to="3,0"
//           dur="1s"
//           begin="0.8s"
//           repeatCount="1"
//           fill="freeze"
//         />
//       </path>
//       <path d="M8 14v7">
//         <animate
//           attributeName="stroke-dasharray"
//           from="0,7"
//           to="7,0"
//           dur="1s"
//           begin="1s"
//           repeatCount="1"
//           fill="freeze"
//         />
//       </path>
//     </svg>
//   );
// };



const Fasie = () => {
  return <svg version="1.1" id="_Слой_1" xmlns="http://www.w3.org/2000/svg" x="0px"
    y="0px" viewBox="0 0 152.5 74"  >
    {/* <style type="text/css">
    .st0{fill:#1B3968;}
    .st1{fill:#FFFFFF;}
    .st2{fill:#E22A2A;}
    .st3{fill:#339FD9;}
    .st4{fill:#E1F1F9;}
  </style> */}
    <path className="st0" fill="#1B3968" d="M13.2,12.3c0,0.5-0.1,1-0.3,1.4s-0.4,0.7-0.8,1c-0.3,0.3-0.7,0.5-1.1,0.6c-0.4,0.1-0.8,0.2-1.2,0.3v1h-2v-1l0,0
    c-0.6-0.1-1.1-0.2-1.5-0.4s-0.8-0.4-1-0.8c-0.3-0.3-0.5-0.7-0.6-1.1c-0.1-0.4-0.2-0.9-0.2-1.4V7.7c0-0.5,0.1-1,0.3-1.4
    c0.2-0.4,0.4-0.7,0.8-1C5.9,5,6.3,4.8,6.7,4.7c0.4-0.1,0.8-0.2,1.2-0.2V3.8h2v0.7c0.4,0,0.8,0.1,1.2,0.2c0.4,0.2,0.7,0.4,1.1,0.6
    c0.3,0.3,0.6,0.6,0.7,1c0.2,0.4,0.3,0.9,0.3,1.4V12.3 M7.9,6C7.5,6.1,7.1,6.3,6.8,6.5C6.6,6.7,6.5,7.1,6.5,7.6v4.9
    c0,0.2,0,0.4,0.1,0.6c0.1,0.2,0.2,0.3,0.3,0.4c0.1,0.1,0.3,0.2,0.5,0.3s0.4,0.1,0.5,0.1V6L7.9,6z M11.4,7.6c0-0.5-0.1-0.8-0.4-1.1
    c-0.3-0.3-0.6-0.4-1-0.4V14c0.2,0,0.3-0.1,0.5-0.1c0.2-0.1,0.3-0.2,0.5-0.3c0.1-0.1,0.3-0.3,0.3-0.4c0.1-0.2,0.1-0.4,0.1-0.6
    L11.4,7.6L11.4,7.6z"/>
    <path className="st0" fill="#1B3968" d="M23.2,13.3c0,1.1-0.3,1.9-0.8,2.5c-0.5,0.6-1.3,0.8-2.4,0.8s-1.9-0.3-2.4-0.8c-0.5-0.5-0.7-1.3-0.7-2.4V7.1
    c0-0.5,0.1-1,0.2-1.4s0.3-0.8,0.5-1.1c0.3-0.3,0.6-0.5,1-0.7C19,3.8,19.4,3.7,20,3.7s1.1,0.1,1.5,0.2c0.4,0.2,0.7,0.4,1,0.7
    c0.2,0.3,0.4,0.7,0.5,1.1s0.2,0.9,0.2,1.4V13.3 M21.1,6.9c0-0.4-0.1-0.7-0.2-1c-0.2-0.3-0.4-0.4-0.9-0.4c-0.2,0-0.4,0-0.6,0.2
    c-0.1,0.1-0.3,0.2-0.4,0.4s-0.1,0.3-0.2,0.5c0,0.2,0,0.4,0,0.5v6.4c0,0.3,0.1,0.6,0.3,0.9c0.2,0.3,0.5,0.4,0.9,0.4s0.7-0.1,0.9-0.4
    c0.2-0.3,0.3-0.6,0.3-1L21.1,6.9L21.1,6.9z"/>
    <polyline className="st0" fill="#1B3968" points="31.3,16.6 31.3,11 29,11 29,16.6 27,16.6 27,3.8 29,3.8 29,9.1 31.3,9.1 31.3,3.8 33.3,3.8 33.3,16.6 
    31.3,16.6 "/>
    <path className="st0" fill="#1B3968" d="M38,16.6V19h-1.7v-4.1l0.8-0.2c0.4-0.4,0.6-0.8,0.7-1.3c0.1-0.5,0.2-1.1,0.2-1.9l0.2-7.7H44v10.9h0.8V19h-1.7
    v-2.4H38 M39.9,11.5c0,0.7-0.1,1.4-0.1,1.9c-0.1,0.5-0.2,1-0.4,1.3h2.8v-9H40L39.9,11.5z"/>
    <path className="st0" fill="#1B3968" d="M57,3.7c0.5,0,1,0.1,1.4,0.2c0.4,0.1,0.7,0.4,1,0.6c0.3,0.3,0.4,0.6,0.6,0.9c0,0.4,0,0.8,0,1.2l-2,0.5
    c0-0.5-0.1-0.8-0.3-1.1c-0.2-0.3-0.5-0.4-0.9-0.4c-0.2,0-0.4,0-0.6,0.1c-0.1,0.1-0.3,0.2-0.3,0.4c-0.1,0.2-0.1,0.3-0.2,0.5
    c0,0.2,0,0.4,0,0.5v6.5c0,0.1,0,0.3,0,0.4c0,0.2,0.1,0.3,0.2,0.4c0.1,0.1,0.2,0.2,0.3,0.3s0.3,0.1,0.5,0.1c0.7,0,1.1-0.5,1.1-1.6
    c0.3,0.1,0.7,0.1,1,0.2s0.7,0.1,1,0.1c0,1-0.3,1.7-0.8,2.3c-0.5,0.6-1.3,0.8-2.3,0.8c-1.1,0-1.9-0.3-2.4-0.8
    c-0.5-0.5-0.8-1.3-0.8-2.3V7.2c0-0.5,0-0.9,0.2-1.4C53.8,5.4,54,5,54.2,4.7c0.3-0.3,0.6-0.6,1-0.7C55.9,3.8,56.4,3.7,57,3.7"/>
    <path className="st0" fill="#1B3968" d="M69.6,13.3c0,1.1-0.3,1.9-0.8,2.5c-0.5,0.6-1.3,0.8-2.4,0.8s-1.9-0.3-2.4-0.8c-0.5-0.5-0.7-1.3-0.7-2.4V7.1
    c0-0.5,0.1-1,0.2-1.4c0.1-0.4,0.3-0.8,0.5-1.1c0.2-0.3,0.6-0.5,1-0.7s0.9-0.3,1.4-0.3c0.6,0,1.1,0.1,1.5,0.2c0.4,0.2,0.7,0.4,1,0.7
    c0.2,0.3,0.4,0.7,0.5,1.1s0.2,0.9,0.2,1.4L69.6,13.3 M67.6,6.9c0-0.4-0.1-0.7-0.2-1c-0.2-0.3-0.4-0.4-0.9-0.4c-0.2,0-0.4,0-0.6,0.2
    c-0.1,0.1-0.3,0.2-0.4,0.4s-0.1,0.3-0.2,0.5c0,0.2,0,0.4,0,0.5v6.4c0,0.3,0.1,0.6,0.3,0.9c0.2,0.3,0.5,0.4,0.9,0.4s0.7-0.1,0.9-0.4
    c0.2-0.3,0.3-0.6,0.3-1L67.6,6.9L67.6,6.9z"/>
    <path className="st0" fill="#1B3968" d="M74.3,16.6V19h-1.7v-4.1l0.8-0.2c0.4-0.4,0.6-0.8,0.7-1.3s0.2-1.1,0.2-1.9l0.2-7.7h5.8v10.9h0.8V19h-1.7v-2.4
    H74.3 M76.1,11.5c0,0.7-0.1,1.4-0.1,1.9c-0.1,0.5-0.2,1-0.4,1.3h2.8v-9h-2.1L76.1,11.5z"/>
    <polyline className="st0" fill="#1B3968" points="84.4,16.6 84.4,3.8 90.2,3.8 90.2,5.6 86.4,5.6 86.4,9.1 89.7,9.1 89.7,11 86.4,11 86.4,14.7 
    90.2,14.7 90.2,16.6 84.4,16.6 "/>
    <path className="st0" fill="#1B3968" d="M98,8.6l-2.7,8h-1.9V3.8h2v8.1h0.1l2.7-8.1h2v12.7h-2L98,8.6L98,8.6 M95.6,0.9c0,0.1,0,0.3,0.1,0.4
    c0.1,0.1,0.2,0.3,0.3,0.4c0.1,0.1,0.2,0.2,0.4,0.3c0.1,0.1,0.3,0.1,0.5,0.1s0.4,0,0.5-0.1s0.3-0.2,0.4-0.3S98,1.5,98,1.3
    s0-0.3,0-0.4h1.5c0,0.2-0.1,0.5-0.2,0.8C99.2,2,99,2.3,98.8,2.6C98.6,2.8,98.3,3,98,3.2s-0.7,0.2-1.1,0.2c-0.5,0-0.9-0.1-1.2-0.2
    C95.3,3,95,2.8,94.8,2.6s-0.4-0.5-0.5-0.8S94.1,1.2,94.1,1L95.6,0.9L95.6,0.9z"/>
    <path className="st0" fill="#1B3968" d="M106.9,3.7c0.5,0,1,0.1,1.4,0.2c0.4,0.1,0.7,0.4,1,0.6c0.3,0.3,0.4,0.6,0.6,0.9c0.1,0.4,0.2,0.7,0.2,1.2l-2,0.5
    c0-0.5-0.1-0.8-0.3-1.1c-0.2-0.3-0.5-0.4-0.9-0.4c-0.2,0-0.4,0-0.6,0.1c-0.1,0.1-0.3,0.2-0.3,0.4c-0.1,0.2-0.1,0.3-0.2,0.5
    c0,0.2,0,0.4,0,0.5v6.5c0,0.1,0,0.3,0,0.4c0,0.2,0.1,0.3,0.2,0.4c0.1,0.1,0.2,0.2,0.3,0.3c0.1,0.1,0.3,0.1,0.5,0.1
    c0.7,0,1.1-0.5,1.1-1.6c0.3,0.1,0.7,0.1,1,0.2s0.7,0.1,1,0.1c0,1-0.3,1.7-0.8,2.3c-0.5,0.6-1.3,0.8-2.3,0.8c-1.1,0-1.8-0.3-2.4-0.8
    s-0.8-1.3-0.8-2.3V7.2c0-0.5,0-0.9,0.2-1.4c0.1-0.4,0.3-0.8,0.5-1.1c0.3-0.3,0.6-0.6,1-0.7C105.8,3.8,106.3,3.7,106.9,3.7"/>
    <polyline className="st0" fill="#1B3968" points="116.6,5.6 116.6,16.6 114.6,16.6 114.6,5.6 112.5,5.6 112.5,3.8 118.7,3.8 118.7,5.6 116.6,5.6 " />
    <path className="st0" fill="#1B3968" d="M126.4,9.9c0.6,0.2,1,0.5,1.2,1s0.3,1.1,0.3,1.8c0,1.3-0.3,2.3-0.9,2.9s-1.5,0.9-2.7,0.9h-2.7V3.8h2.8
    c1.1,0,2,0.3,2.5,0.9s0.8,1.5,0.8,2.6c0,0.6-0.1,1.1-0.3,1.5C127.3,9.2,126.9,9.6,126.4,9.9L126.4,9.9 M125.9,7.3
    c0-0.6-0.1-1-0.4-1.3c-0.2-0.3-0.6-0.4-1.2-0.4h-0.6v3.3h0.7c0.5,0,0.9-0.1,1.1-0.4C125.7,8.2,125.9,7.8,125.9,7.3 M126,12.5
    c0-0.6-0.1-1-0.3-1.3s-0.6-0.4-1.2-0.4h-0.8v4h0.8c0.3,0,0.6-0.1,0.8-0.2c0.2-0.1,0.3-0.3,0.4-0.5s0.2-0.4,0.2-0.7
    C126,13.1,126,12.8,126,12.5"/>
    <polyline className="st0" fill="#1B3968" points="136.2,8.6 133.5,16.6 131.6,16.6 131.6,3.8 133.5,3.8 133.5,11.9 133.6,11.9 136.3,3.8 138.3,3.8 
    138.3,16.6 136.3,16.6 136.3,8.6 136.2,8.6 "/>
    <path className="st0" fill="#1B3968" d="M141.6,16.6l0.7-4c0.1-0.4,0.2-0.8,0.4-1.2s0.4-0.6,0.8-0.8c-0.6-0.2-1-0.6-1.2-1.1S142,8.4,142,7.7
    c0-1.3,0.3-2.2,0.9-2.9c0.6-0.6,1.5-1,2.5-1h3v12.7h-2v-5.4h-0.5c-0.4,0-0.7,0.1-1,0.4c-0.3,0.3-0.5,0.7-0.6,1.5l-0.6,3.6H141.6
     M145.6,5.6c-0.6,0-1,0.2-1.3,0.5s-0.4,0.8-0.4,1.4s0.2,1.1,0.5,1.3s0.7,0.4,1.2,0.4h0.8V5.6H145.6L145.6,5.6z"/>
    <polyline className="st0" fill="#1B3968" points="10.6,27.2 7.2,37.1 4.9,37.1 4.9,21.3 7.3,21.3 7.3,31.3 7.4,31.3 10.7,21.3 13.1,21.3 13.1,37.1 
    10.7,37.1 10.7,27.2 10.6,27.2 "/>
    <polyline className="st0" fill="#1B3968" points="25.3,37.1 25.3,30.2 22.5,30.2 22.5,37.1 20,37.1 20,21.3 22.5,21.3 22.5,27.9 25.3,27.9 25.3,21.3 
    27.8,21.3 27.8,37.1 25.3,37.1 "/>
    <polyline className="st0" fill="#1B3968" points="40.1,37.1 40.1,30.2 37.2,30.2 37.2,37.1 34.8,37.1 34.8,21.3 37.2,21.3 37.2,27.9 40.1,27.9 
    40.1,21.3 42.6,21.3 42.6,37.1 40.1,37.1 "/>
    <path className="st0" fill="#1B3968" d="M57.2,33c0,1.4-0.3,2.4-0.9,3.1c-0.6,0.7-1.6,1.1-3,1.1s-2.3-0.3-2.9-1c-0.6-0.7-0.9-1.6-0.9-2.9v-8
    c0-0.6,0.1-1.2,0.2-1.7c0.1-0.5,0.4-1,0.7-1.3c0.3-0.4,0.7-0.7,1.2-0.8c0.5-0.2,1.1-0.3,1.8-0.3s1.4,0.1,1.8,0.3
    c0.5,0.2,0.9,0.5,1.2,0.8c0.3,0.4,0.5,0.8,0.7,1.3c0.1,0.5,0.2,1.1,0.2,1.8L57.2,33 M54.7,25.1c0-0.4-0.1-0.8-0.3-1.2
    c-0.2-0.4-0.6-0.5-1.1-0.5c-0.3,0-0.5,0.1-0.7,0.2s-0.3,0.3-0.4,0.5c-0.1,0.2-0.2,0.4-0.2,0.6s-0.1,0.4-0.1,0.7v8
    c0,0.4,0.1,0.8,0.3,1.1s0.6,0.5,1.1,0.5s0.9-0.2,1.1-0.5c0.2-0.4,0.3-0.8,0.3-1.2L54.7,25.1L54.7,25.1z"/>
    <path className="st0" fill="#1B3968" ld="M69.8,28.8c0.7,0.2,1.2,0.7,1.5,1.3c0.3,0.6,0.4,1.4,0.4,2.2c0,1.7-0.4,2.9-1.1,3.6c-0.7,0.8-1.8,1.2-3.3,1.2
    H64V21.2h3.5c1.4,0,2.4,0.4,3.1,1.1s1,1.8,1,3.2c0,0.7-0.1,1.4-0.4,1.9s-0.7,1-1.3,1.2L69.8,28.8 M69.1,25.6c0-0.7-0.1-1.3-0.4-1.6
    c-0.3-0.3-0.8-0.5-1.5-0.5h-0.8v4.1h0.8c0.7,0,1.1-0.2,1.4-0.5C69,26.7,69.1,26.2,69.1,25.6 M69.3,32.1c0-0.7-0.1-1.3-0.4-1.7
    c-0.3-0.4-0.8-0.5-1.5-0.5h-0.9v4.9h1c0.4,0,0.7-0.1,1-0.2c0.2-0.1,0.4-0.4,0.6-0.6c0.1-0.2,0.2-0.5,0.3-0.8
    C69.3,32.8,69.3,32.4,69.3,32.1"/>
    <path className="st0" fill="#1B3968" d="M84.2,37.1l-0.6-3.3h-2.9c-0.1,0.5-0.2,1.1-0.3,1.6c-0.1,0.6-0.2,1.1-0.3,1.7h-2.5c0.5-2.7,1.1-5.3,1.6-7.9
    s1.1-5.3,1.6-7.9h2.8l3.3,15.8H84.2 M82,24.9c-0.2,1.1-0.3,2.2-0.5,3.3c-0.2,1.1-0.4,2.2-0.6,3.3h2.2L82,24.9
    C82.1,24.9,82,24.9,82,24.9z"/>
    <polyline className="st0" fill="#1B3968" points="100.5,21.3 100.5,34.8 101.5,34.8 101.5,40.1 99.4,40.1 99.4,37.1 92.7,37.1 92.7,21.3 95.2,21.3 
    95.2,34.8 98,34.8 98,21.3 100.5,21.3 "/>
    <polyline className="st0" fill="#1B3968" points="113.2,27.2 109.9,37.1 107.4,37.1 107.4,21.3 109.9,21.3 109.9,31.3 110,31.3 113.3,21.3 115.7,21.3 
    115.7,37.1 113.3,37.1 113.3,27.2 113.2,27.2 "/>
    <path className="st0" fill="#1B3968" d="M122,37.1l0.9-5c0.1-0.5,0.3-1,0.5-1.5c0.2-0.4,0.6-0.8,1-1c-0.7-0.3-1.2-0.8-1.5-1.4s-0.4-1.4-0.4-2.2
    c0-1.6,0.4-2.7,1.1-3.6c0.8-0.8,1.8-1.2,3.1-1.2h3.7V37h-2.5v-6.8h-0.7c-0.4,0-0.8,0.2-1.2,0.5c-0.4,0.3-0.6,0.9-0.8,1.8l-0.8,4.5
    L122,37.1 M126.9,23.6c-0.7,0-1.3,0.2-1.6,0.7c-0.3,0.4-0.4,1-0.4,1.8s0.2,1.3,0.6,1.6c0.4,0.3,0.9,0.4,1.5,0.4h1v-4.5
    C127.9,23.6,126.9,23.6,126.9,23.6z"/>
    <polyline className="st0" fill="#1B3968" points="141.9,33.6 140.8,30.7 139.8,27.9 139.7,27.9 139.7,37.1 137.2,37.1 137.2,21.3 139.6,21.3 
    141.3,25.8 142.6,29.2 142.7,29.2 144,25.7 145.7,21.3 148.1,21.3 148.1,37.1 145.6,37.1 145.6,28 145.6,28 144.5,30.8 143.4,33.6 
    141.9,33.6 "/>
    <path className="st1" d="M146.7,45.1c0.5-0.3,1-0.7,1.5-1h0.1l0,0l0,0c-2.8,1.3-5.6,2.4-8.3,3.5c-17.2,6.7-33.3,9.5-47.4,9.6
    C72.8,57.3,57.2,52,48.9,44h-15C56.4,65.3,108.9,70.2,146.7,45.1"/>
    <path className="st2" d="M19.1,44H4.9c19,20.1,46.2,27.7,69,28.7c2.4,0.1,4.7,0.1,6.9,0.1c14.8-0.2,31.3-3.4,45.6-11.1
    c8.2-4.4,15.7-10.2,21.8-17.8l0,0C113.6,77.9,44.3,72,19.1,44"/>
    <path className="st3" d="M148.2,44L148.2,44L148.2,44L148.2,44c-0.6,0.4-1.1,0.8-1.6,1.1C108.9,70.2,56.4,65.3,33.9,44H19.1
    C44.3,72,113.6,77.9,148.2,44L148.2,44"/>
    <path className="st4" d="M148.6,43.9L148.6,43.9c0-0.1-0.1-0.2-0.2-0.2c0,0-0.1-0.1-0.2-0.1h-0.1H148h-0.1c-2.8,1.2-5.5,2.4-8.3,3.5
    c-15.9,6.1-32.2,9.4-47.2,9.5h-0.8c-18.4,0-33.8-4.7-42.3-12.9l-0.1-0.1l-0.1-0.1H49H5c0,0-0.1,0-0.2,0.1l-0.2,0.1
    c-0.1,0.1-0.2,0.2-0.1,0.3v0.2c0,0,0,0.1,0.1,0.2c8.8,9.3,20,16.5,33.2,21.5c11,4.2,23.5,6.6,36,7.2c1.7,0.1,3.5,0.1,5.3,0.1
    c0.6,0,1.1,0,1.7,0c7.8-0.1,15.6-1,23.1-2.8c8.2-1.9,15.8-4.7,22.7-8.3c8.8-4.7,16.1-10.6,21.9-17.8l0,0l0,0v-0.1v-0.1V44
    C148.6,44.1,148.6,44,148.6,43.9L148.6,43.9z M147,44.9c-5.6,6.6-12.6,12.2-20.8,16.5c-12.9,6.8-29,10.7-45.4,11
    c-2.3,0-4.6,0-6.9-0.1C57.8,71.6,27.6,67,5.7,44.4h43.1c8.7,8.3,24.3,13,42.8,13c0.3,0,0.5,0,0.8,0c15.1-0.1,31.5-3.4,47.4-9.6
    C142.3,47,144.7,46,147,44.9L147,44.9z"/>
  </svg>

}